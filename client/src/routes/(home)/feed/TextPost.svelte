...
<div
  class="card bg-base-100 mb-0 mt-0 border-0"
>
<div class="m-3 ml-3" on:click={() => window.location.replace(`/profile/${PostData[1]}`)}>
  <img class="mr-3 w-7 mask mask-circle float-left" src="https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp" />
  <div class="mt-1">
  ...
  <div class="m-3 mt-0">
    ...
    <h1 class="card-title mb-1 text-2xl">{PostData[2]}</h1>
    {#if PostData[4]!="" && PostData[4]!=null}
      <div class="badge badge-secondary badge-outline">{PostData[4]}</div>
    {/if}
    <div class="flex flex-row">
      {#if PostData[3]!=""}
      {#each PostData[3].split(",") as tag}
      <span class="badge badge-accent badge-outline  mr-1 mt-1 mb-5">{tag}</span>
      {/each}
      {/if}
    </div>
    {#if $page.url.pathname.includes("/post/")}
      <p class="mb-0 mt-3 break-words">{PostData[8]}</p>
    {:else}
      <p class="mb-0 max-h-36 break-words overflow-hidden [mask:linear-gradient(0deg,#0000,#000_1.25rem)] md:[mask:linear-gradient(0deg,#0000,#000_1.5rem)]">{PostData[8]}</p>
    {/if}
  </a>
  </div>
  <div class="card-body p-1">
    <div class="card-actions">
      <button class="btn btn-ghost btn-square w-8 {curReaction == 1 ? 'text-accent' : ''}" on:click={() => react(PostData[0], 1)}>
        <svg
          stroke-width="0"
