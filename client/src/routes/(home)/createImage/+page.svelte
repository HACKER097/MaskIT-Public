...
<div class="mt-4 flex items-center justify-center flex-col">
  <div class="card w-11/12 bg-base-100 shadow-xl h-11/12">
      <figure>
          ...
          {:else}
              <img src="https://daisyui.com/images/stock/photo-1606107557195-0e29a4b5b4aa.jpg" alt="Default Image" />
          {/if}
      </figure>
      <div class="card-body">
          {#if loc !== ""}
              <div class="badge badge-secondary badge-outline 0">{loc}</div>
          {/if}
          {#if seltags.length > 0}
              <div class="flex flex-row">
                  {#each seltags as seltag}
                      <span class="badge badge-accent badge-outline mb-2 mr-1">{seltag}</span>
                  {/each}
              </div>
          {/if}
          <div class="card-actions justify-end">
            <input bind:files={ImageData} type="file" accept="image/*" class="file-input file-input-bordered file-input-primary w-full" on:change={handleFileChange} />              <input bind:value={title} type="text" placeholder="Caption" class="input input-bordered input-primary w-full mt-4" />
              ...
              <button class="btn btn-primary mt-4 w-full mb-16" on:click={() => post()}>
                  {#if !loading}
                      POST
                  {:else}
